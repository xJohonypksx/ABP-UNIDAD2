            ...
            <div class="col-md-10">
                <div class="rounded shadow-lg bg-gradient-to-b from-gray-800 to-gray-900 p-4">
                    <table class="table table-hover text-white align-middle">
                        ...
                        <tr>
                            <th class="fw-bold text-uppercase text-center" style="letter-spacing: 1px;">ID Inventario</th>
                            <th class="fw-bold text-uppercase text-center" style="letter-spacing: 1px;">ID Producto</th>
                            <th class="fw-bold text-uppercase text-center" style="letter-spacing: 1px;">Cantidad Disponible</th>
                            <th class="fw-bold text-uppercase text-center" style="letter-spacing: 1px;">Ubicación</th>
                        </tr>
                        </thead>
                        <tbody class="bg-gray-800">
                        @foreach($inventarios as $inventario)
                            <tr class="border-bottom border-gray-700">
                                <td class="text-center text-primary fs-4">{{ $inventario->id_inventario }}</td>
                                <td class="text-center fs-5">{{ $inventario->id_producto }}</td>
                                <td class="text-center fs-5">{{ $inventario->cantidad_dis }}</td>
                                <td class="text-center fs-5">{{ $inventario->ubicacion }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
